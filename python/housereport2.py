...
for item in pjreo_houses:
    id1 = item['_id']
    url = 'http://localhost:9900/CustomService/ReportManager/GetReportHouse/?FormName=Form21PrintView&DocumentId='
    url += id1
    a = s.get(url)
    filepath = "F:/out/21_"
    ...
    f.write(a.content)
    f.close()
    url = 'http://localhost:9900/CustomService/ReportManager/GetReportHouse/?FormName=Form28PrintView&DocumentId='
    url += id1
    a = s.get(url)
    filepath = "F:/out/28_"
    filepath += item['full_address']['street_formal_name'] + '_'
